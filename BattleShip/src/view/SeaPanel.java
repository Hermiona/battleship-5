...
public class SeaPanel extends JPanel {
    ...
    //if display for computer player, they need the mouse listener to listen which button the player click on to aattack
    //get the string in the util.ApplicationConstants
    public SeaPanel(String displaySide, Ship[] ships, ShipCellList shipCellList, SharedObject turn) {
        ...
        if (displaySide.equals(ApplicationConstants.PLAYER_COMPUTER)) {
            //init the controller
...
//            BufferedImage decorationImage = ImageIO.read(new File("images/shark1.png"));
//            g.drawImage(decorationImage, 300, 100, 50, 50, null);
//            decorationImage = ImageIO.read(new File("images/shark2.png"));
//            g.drawImage(decorationImage, 200, 300, 50, 50, null);
            
            //check if the panel is displaying for human side then draw the ship images
            if (this.displaySide.equals(ApplicationConstants.PLAYER_HUMAN)) {
            ...
            if(!hitPositionList.isEmpty()){
                BufferedImage fireImage = ImageIO.read(new File("images/hit.png"));
            ...
            if(!missPositionList.isEmpty()){
                BufferedImage fireImage = ImageIO.read(new File("images/miss.png"));
